...
class RequestsKeywords(object):
    ...
    def _format_data_according_to_header(self, session, data, headers):
        ...
        if data is not None and headers is not None and 'Content-Type' in headers and self._is_json(data):
            if headers['Content-Type'].find("application/json") != -1:
                if not isinstance(data, types.GeneratorType):
                    data = json.dumps(data)
                    # data = self._utf8_urlencode(data)
                    logger.info("From here 1 :::: "+data)
            elif headers['Content-Type'].find("application/x-www-form-urlencoded") != -1:
                data = self._utf8_urlencode(data)
                logger.info("From here 2 :::: "+data)
        else:
            data = json.dumps(data)
            logger.info("From here 3 :::: "+data)
    ...
    def inject_lists(self, expected, actual):
        if len(expected) != len(actual):
            raise AssertionError("Lists have different lenghts")
        [
            self.response_should_contain(expected[index], actual[index])
            for index in range(1, len(expected))
        ]

    def inject_dictionaries(self, expected, actual):
        [
            self.response_should_contain(expected[key], actual[key])
            for key in expected.keys()
        ]

    def response_should_contain(self, expected, actual, debug=False):
        if debug:
            logger.info(f"Comparing {expected} to {actual}")
            logger.info(f"Comparing {type(actual)} and {type(expected)}")
        ...
        if not expected:
            ...
            self.inject_lists(expected, actual)
        elif type(expected) in [dict, DotDict]:
            self.inject_dictionaries(expected, actual)
        elif str(expected) != str(actual):
            raise AssertionError(f"{expected} was expected but {actual} was found.")
